...
## MB seperation
def MBseparate_freq(data, w = [1,25], wn = 7):
    window = np.hanning(wn)
    temp = np.reshape(data, (data.shape[0]*data.shape[1],data.shape[2]))
    tempfft = np.fft.fftshift(np.fft.fft(temp,axis=1))
    pos = np.zeros(int(data.shape[2]/2))
    neg = np.zeros(int(data.shape[2]/2))
    for i in range(int(-data.shape[2]/2),int(data.shape[2]/2)):
        ii = int(np.linalg.norm(i))
        if i > 0:
            if ii > w[0] and ii <= w[1]:
                pos[i] = 1
        else:
            if ii > w[0] and ii <= w[1]:
                neg[i] = 1
    ...
    pos = pos/np.max(pos)
    neg = np.convolve(neg,window,'same')
    neg = neg/np.max(neg)
    filter = np.concatenate((neg,pos))
    for i in range(tempfft.shape[0]):
        tempfft[i,:] = tempfft[i,:]*filter
    tempifft = np.fft.ifft(np.fft.fftshift(tempfft),axis=1)
    return np.reshape(tempifft.real, (data.shape[0],data.shape[1],data.shape[2]))
